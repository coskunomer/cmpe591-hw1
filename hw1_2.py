...
def train():
    ...
    plt.plot(range(1, len(train_losses) + 1), train_losses, label="CNN Train Loss", marker="o")
    plt.plot(range(1, len(test_losses) + 1), test_losses, label="CNN Test Loss", marker="o")
...
def test():
    model = CNN()
    model.load_state_dict(torch.load("hw1_2.pt"))
    model.eval()
